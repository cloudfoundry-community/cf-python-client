...
class IsolationSegmentManager(EntityManager):
    ...
    def update(self, isolation_segment_guid: str, name: str,
               meta_labels: Optional[dict] = None, meta_annotations: Optional[dict] = None) -> Entity:
        data = {
            'name': name,
            'metadata': {
    ...
    def entitle_organizations(self, isolation_segment_guid: str, *org_guids: str) -> ToManyRelationship:
        ...
        return ToManyRelationship.from_json_object(super(IsolationSegmentManager, self)._post(
            '%s%s/%s/relationships/organizations' % (self.target_endpoint, self.entity_uri, isolation_segment_guid),
            data=data))

    def list_entitled_organizations(self, isolation_segment_guid: str, ) -> ToManyRelationship:
        return ToManyRelationship.from_json_object(super(IsolationSegmentManager, self)._get(
            '%s%s/%s/relationships/organizations' % (self.target_endpoint, self.entity_uri, isolation_segment_guid)))

    def list_entitled_spaces(self, isolation_segment_guid: str, ) -> ToManyRelationship:
        return ToManyRelationship.from_json_object(super(IsolationSegmentManager, self)._get(
    ...
    def revoke_organization(self, isolation_segment_guid: str, org_guid: str):
        super(IsolationSegmentManager, self)._delete('%s%s/%s/relationships/organizations/%s' % (self.target_endpoint,
                                                                                                 self.entity_uri,
