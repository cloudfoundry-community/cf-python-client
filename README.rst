...
Or you make use of the cf cli config file generated by `cf login`, which allows for authenticating via SSO and LDAP:

.. code-block:: python

    # init with endpoint & token from the cf cli config file
    from cloudfoundry_client.client import CloudFoundryClient
    target_endpoint = 'from_cf_config'
    client = CloudFoundryClient(target_endpoint, proxy=proxy, verify=False)
    # or - with an alternative path
    client = CloudFoundryClient(target_endpoint, proxy=proxy, verify=False, cf_config_path="some/path/config.json")
