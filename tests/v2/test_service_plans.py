...
class TestServicePlans(unittest.TestCase, AbstractTestCase):
    ...
    def test_list(self):
        self.client.get.return_value = self.mock_response(
            "/v2/service_plans?q=service_guid%3Aservice_id", HTTPStatus.OK, None, "v2", "service_plans", "GET_response.json"
        )
    ...
    def test_get(self):
        self.client.get.return_value = self.mock_response(
            "/v2/service_plans/plan_id", HTTPStatus.OK, None, "v2", "service_plans", "GET_{id}_response.json"
        )
    ...
    def test_list_instances(self):
        self.client.get.return_value = self.mock_response(
            ...
            "v2",
            "apps",
            "GET_{id}_routes_response.json",
        )
        cpt = reduce(
            lambda increment, _: increment + 1, self.client.v2.service_plans.list_instances("plan_id", space_guid="space_id"), 0
        )
        self.client.get.assert_called_with(self.client.get.return_value.url)
        self.assertEqual(cpt, 1)
    ...
    def test_entity(self):
        self.client.get.side_effect = [
            self.mock_response("/v2/service_plans/plan_id", HTTPStatus.OK, None, "v2", "service_plans", "GET_{id}_response.json"),
            self.mock_response(
    ...
    def test_main_list_service_plans(self):
        with patch("cloudfoundry_client.main.main.build_client_from_configuration", new=lambda: self.client):
            self.client.get.return_value = self.mock_response(
                "/v2/service_plans", HTTPStatus.OK, None, "v2", "service_plans", "GET_response.json"
            )
            main.main()
            self.client.get.assert_called_with(self.client.get.return_value.url)
