...
class TestPolicies(unittest.TestCase, AbstractTestCase):
    ...
    def test_delete(self):
        self.client.delete.return_value = self.mock_response("/networking/v1/external/policies/delete", HTTPStatus.OK, None)
        policy = Policy(
            src_id="1081ceac-f5c4-47a8-95e8-88e1e302efb5",
            dst_id="38f08df0-19df-4439-b4e9-61096d4301ea",
            proto="tcp",
            start_port=1234,
            end_port=1234,
        )
        self.client.networking_v1_external.policies.delete(
            [
                policy,
            ]
        )
        self.client.delete.assert_called_with(
            {
                "policies": [
                    {
                        "source": {"id": "1081ceac-f5c4-47a8-95e8-88e1e302efb5"},
                        "destination": {
