...
class TestApps(unittest.TestCase):
    def test_list(self):
        ...
        for application in client.v2.apps.list(space_guid=client.space_guid):
            ...
            if cpt == 0:
                ...
                self.assertIsNotNone(client.v2.apps.get_first(space_guid=client.space_guid, name=application["entity"]["name"]))
    ...
    def test_start(self):
        ...
        _logger.debug("start - %s", client.v2.apps.start(client.app_guid, asynchronous=False))

    def test_stop(self):
        client = build_client_from_configuration()
        _logger.debug("stop - %s", client.v2.apps.stop(client.app_guid, asynchronous=False))
