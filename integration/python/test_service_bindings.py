...
class TestServiceBinding(unittest.TestCase):
    ...
    def test_get(self):
        client = build_client_from_configuration()
        cpt = 0
        for binding in client.service_bindings.list():
            if cpt == 0:
                _logger.debug(binding)
                self.assertIsNotNone(
                    client.service_bindings.get_first(service_instance_guid=binding['entity']['service_instance_guid']))
                self.assertIsNotNone(
                    client.service_bindings.get_first(app_guid=binding['entity']['app_guid']))
                self.assertIsNotNone(
                    client.service_bindings.get(binding['metadata']['guid']))
            cpt += 1
