...
class TestRoutes(unittest.TestCase, AbstractTestCase):
    ...
    def test_list(self):
        self.client.get.return_value = self.mock_response(
            "/v2/routes?q=organization_guid%3Aorganization_guid", HTTPStatus.OK, None, "v2", "routes", "GET_response.json"
        )
        cpt = reduce(lambda increment, _: increment + 1, self.client.v2.routes.list(organization_guid="organization_guid"), 0)
        self.client.get.assert_called_with(self.client.get.return_value.url)
        self.assertEqual(cpt, 1)

    def test_get(self):
        self.client.get.return_value = self.mock_response(
            "/v2/routes/route_id", HTTPStatus.OK, None, "v2", "routes", "GET_{id}_response.json"
        )
    ...
    def test_entity(self):
        self.client.get.side_effect = [
            self.mock_response("/v2/routes/route_id", HTTPStatus.OK, None, "v2", "routes", "GET_{id}_response.json"),
            self.mock_response(
            ...
            self.mock_response(
                "/v2/spaces/b3f94ab9-1520-478b-a6d6-eb467c179ada", HTTPStatus.OK, None, "v2", "spaces", "GET_{id}_response.json"
            ),
            self.mock_response(
                "/v2/routes/75c16cfe-9b8a-4faf-bb65-02c713c7956f/apps", HTTPStatus.OK, None, "v2", "apps", "GET_response.json"
            ),
        ]
    ...
    def test_main_list_routes(self):
        with patch("cloudfoundry_client.main.main.build_client_from_configuration", new=lambda: self.client):
            self.client.get.return_value = self.mock_response(
                "/v2/routes", HTTPStatus.OK, None, "v2", "routes", "GET_response.json"
            )
            main.main()
            self.client.get.assert_called_with(self.client.get.return_value.url)
    ...
    def test_main_get_route(self):
        with patch("cloudfoundry_client.main.main.build_client_from_configuration", new=lambda: self.client):
            self.client.get.return_value = self.mock_response(
                "/v2/routes/75c16cfe-9b8a-4faf-bb65-02c713c7956f", HTTPStatus.OK, None, "v2", "routes", "GET_{id}_response.json"
            )
            main.main()
            self.client.get.assert_called_with(self.client.get.return_value.url)
