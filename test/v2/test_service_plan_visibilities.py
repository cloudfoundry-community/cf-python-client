...
class TestServicePlanVisibilities(unittest.TestCase, AbstractTestCase):
    ...
    def test_list(self):
        ...
        cpt = reduce(
            lambda increment, _: increment + 1, self.client.v2.service_plan_visibilities.list(space_guid="space_guid"), 0
        )
    ...
    def test_get(self):
        self.client.get.return_value = self.mock_response(
            "/v2/service_plan_visibilities/guid", HTTPStatus.OK, None, "v2", "service_plan_visibilities", "GET_{id}_response.json"
        )
    ...
    def test_create(self):
        ...
        self.client.post.assert_called_with(
            ...
            json=dict(service_plan_guid="service_plan_guid", organization_guid="organization_guid"),
        )
        self.assertIsNotNone(service_plan_visibilities)

    def test_update(self):
        self.client.put.return_value = self.mock_response(
            "/v2/service_plan_visibilities/guid", HTTPStatus.OK, None, "v2", "service_plan_visibilities", "PUT_{id}_response.json"
        )
        service_plan_visibilities = self.client.v2.service_plan_visibilities.update(
            "guid", service_plan_guid="service_plan_guid", organization_guid="organization_guid"
        )
    ...
    def test_delete(self):
        self.client.delete.return_value = self.mock_response("/v2/service_plan_visibilities/guid", HTTPStatus.NO_CONTENT, None)
    ...
    @patch.object(
        ...
        [
            "main",
            "create_service_plan_visibility",
            json.dumps(dict(service_plan_guid="service-plan-id", organization_guid="organization-id")),
        ],
    )
    ...
    def test_main_list_service_plan_visibilities(self):
        with patch("cloudfoundry_client.main.main.build_client_from_configuration", new=lambda: self.client):
            self.client.get.return_value = self.mock_response(
                "/v2/service_plan_visibilities", HTTPStatus.OK, None, "v2", "service_plan_visibilities", "GET_response.json"
            )
            main.main()
            self.client.get.assert_called_with(self.client.get.return_value.url)
